 ...
 */
public class UtilizationModelCBSInIOPS implements UtilizationModel {
	 ...
	 */
	public UtilizationModelCBSInIOPS(String inputPath, double schedulingInterval)
		...
		for (int i = 0; i < n - 1; i++) {
			...
			//#得到工作负载数组，0：写iops 1：读iops 2：写带宽 3：读带宽
			double thr=100;
			double workload=Double.valueOf(valuelist[3]);
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public UtilizationModelCBSInIOPS(String inputPath, double schedulingInterval, int dataSamples)
			throws NumberFormatException,
			IOException {
		setSchedulingInterval(schedulingInterval);
