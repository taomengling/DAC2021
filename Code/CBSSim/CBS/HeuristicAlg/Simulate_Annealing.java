...
/**
 * 单目标(只以负载均衡为目标的优化)的模拟退火算法
 */
public class Simulate_Annealing {
    ...
    public Simulate_Annealing(List<CloudDisk> diskList,List<Warehouse> warehouseList) {
     ...
     * @return 重新布局后的系统均衡度
     */
    public double Balance_Fitness(int[] map_array) {
        double qos_fit = 0;
        double cost_fit = 0;
...
//            double[] cd_res_use_vec = migration_list.get(i).getAllDimUtilizationMean(); // 该云盘的资源使用向量
    ...
    public double updataBalance(CloudDisk cd,int pre_index,int dest_index){
        ...
        double[] system_res_util_vec = new double[warehouse_util_state[0].length]; // 系统平均资源利用率向量
        int warehouse_num = warehouse_util_state.length;
        for(int i=0;i<warehouse_num;i++){
            system_res_util_vec[0] += warehouse_util_state[i][0]/warehouse_num;
            system_res_util_vec[1] += warehouse_util_state[i][1]/warehouse_num;
            system_res_util_vec[2] += warehouse_util_state[i][2]/warehouse_num;
            system_res_util_vec[3] += warehouse_util_state[i][3]/warehouse_num;
            system_res_util_vec[4] += warehouse_util_state[i][4]/warehouse_num;
        }
//        double[] system_res_util_vec = getBalanceStandard(this.warehouseList);
     ...
     * @return 最优的迁移map
     */
    public int[] minimizeLoadBalance(){
        ...
        double best_fitness = Balance_Fitness(current_map); // 记录探索过程中的最优适应度
        ...
        while (annel_time <T_desc) { // 小于降温的次数 不断循环降温
            ...
            while(iteration < iteration_limit) { //每次降温迭代相应次数
                ...
                current_fitness = Balance_Fitness(random_map); // 计算 当前探索解的适应度
